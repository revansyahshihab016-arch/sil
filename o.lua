...
do
    ...
    spidermanSection:keybind({
        name = "spiderman keybind",
        flag = "spiderman_keybind",
        default = Enum.KeyCode.B,
        display = "spiderman",
        callback = function(active)
            if not cfg['spiderman']['enabled'] then return end
            cfg['spiderman']['enabled'] = not cfg['spiderman']['enabled']
            library:notification({ text = cfg['spiderman']['enabled'] and "spiderman on" or "spiderman off" })
        end,
    })
    ...
    speedSection:keybind({
        ...
        display = "speed",
        callback = function(active)
            if not cfg['speed modifications']['enabled'] then return end
            speedenabled = not speedenabled
            if not speedenabled then
                local hum = localplayer.Character and localplayer.Character:FindFirstChildOfClass("Humanoid")
                if hum then hum.WalkSpeed = 16 end
            end
            library:notification({ text = speedenabled and "speed on" or "speed off" })
        end,
    })
    ...
    superjumpSection:keybind({
        name = "super jump keybind",
        flag = "superjump_keybind",
        default = Enum.KeyCode.G,
        display = "super jump",
        callback = function(active)
            if not cfg['super jump']['enabled'] then return end
            local sjactive = not (cfg['super jump']['_active'] or false)
            cfg['super jump']['_active'] = sjactive
            local hum = localplayer.Character and localplayer.Character:FindFirstChildOfClass("Humanoid")
            if hum then
                hum.JumpPower = sjactive and cfg['super jump']['jump power'] or 50
            end
            library:notification({ text = sjactive and "super jump on" or "super jump off" })
        end,
    })
...
runservice.RenderStepped:Connect(function(dt)
    ...
    if cfg['super jump']['enabled'] and cfg['super jump']['_active'] then
